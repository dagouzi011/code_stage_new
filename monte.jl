# Complete SDE Stochastic Algae Control Model with OptimalControl Inputs and Monte Carlo Simulations
...
σ = 0.2
tf = 100.0
num_trials = 100  # Number of Monte Carlo simulations
...
for trial in 1:num_trials
    ...
    vp_vals = [vP(sol_sde[i][2], sol_sde[i][3]) for i in 1:length(sol_sde)]
    integral_vp = sum(vp_vals) * (sol_sde.t[2] - sol_sde.t[1])
    push!(results, integral_vp)
end

println("Mean ∫vP over ", num_trials, " simulations: ", mean(results))
println("Standard deviation: ", std(results))

# ------------------ Example Visualization (one realization) ------------------
u0 = [0.2, 0.2, 20.0]
prob = SDEProblem(f!, g!, u0, (0.0, tf))
sol_sde = solve(prob, EM(), dt=0.01)
